...
- `key_hold` → WS `Press` … `Release`. **Safety rule: a `Press` without its `Release` freezes the TV's whole remote channel**, so the release is guaranteed without trusting the client: explicit `ReleaseKey`, the next key superseding it, or a **watchdog** (`holdMax`, 10 s) — whichever first. A `Release` is sent even when nothing is tracked as held (extra releases are harmless; missed ones are not).
...
- `Poll` → REST `/api/v2/` reachability as the **live power signal** (reachable ⇒ on), like the bulb's `getPilot` poll, so out-of-band on/off (physical remote) shows up next tick. Online whenever the address resolves (off ≠ offline, so the power toggle stays usable to wake it). A short grace window after an explicit On/Off avoids flicker while the TV powers down. While reachable it also reads volume + mute over UPnP and keeps the event socket connected.
