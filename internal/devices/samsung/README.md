...
- `Poll` → REST `/api/v2/` **`device.PowerState`** as the live power signal (`"on"` ⇒ on; `"standby"` ⇒ off — a TV answering REST from network standby no longer reads as on; older firmware without the field falls back to reachable ⇒ on). Out-of-band on/off (physical remote) shows up next tick. **Always Online** — power-on is WoL by MAC alone, no IP; tying Online to IP resolution would mark an off TV offline once its ARP entry expires and disable the one control that still works (off ≠ offline). A short grace window after an explicit On/Off avoids flicker while the TV powers down. While on it also reads volume + mute over UPnP and keeps the event socket connected.
