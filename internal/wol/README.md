...
- `Send(mac string) error` — builds the magic packet (6×`0xff` + MAC×16) and
  sprays it at the limited broadcast plus every interface's directed broadcast,
  on the two common WoL ports (9 and 7). Reuses `resolver.NormalizeMAC` for the
  MAC. Returns an error if the MAC is malformed or no broadcast target was
  reachable; callers add their own identity with `%w`.
